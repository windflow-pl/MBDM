...
%     end
    [FA] = calc_FA(t,q_out,q_prime_out,nBody,body);
    % ========== asign Applied Moments vector ==========
%     for i=1:3*nBody
%         MA(i,1) = 0.0;
%     end
    [MA] = calc_MA(t,q,q_prime,nBody);
    MOM = MA - sc_om*J*om;
%     MOM
%     sc_om
%     om
%     J
    ...
    % =====================================================================
    for i=1:7*nBody
        if (abs(q_prime_prime_out(i,1))<(10^(-10)))
            q_prime_prime_out(i,1) = 0.0;
        end
        if (abs(q_prime_out(i,1))<(10^(-10)))
            q_prime_out(i,1) = 0.0;
        end
        if (abs(q_out(i,1))<(10^(-10)))
            q_out(i,1) = 0.0;
        end
    end
